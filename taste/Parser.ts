...
if (this.isLaTokenKindEqualTo(14) || this.isLaTokenKindEqualTo(15) || this.isLaTokenKindEqualTo(16)) {
	...
	Factor():number  {
		let type:number ;
		let n:number;let obj_:Obj;let  name:string; 
		type = this.undef; 
		if (this.isLaTokenKindEqualTo(1)) {
			name = this.Ident();
			obj_ =this.tab.Find(name); type = obj_.type;
			...
			this.Get();
			this.gen.Emit(Op.CONST, 1); type = this.boolean; 
		} else if (this.isLaTokenKindEqualTo(6)) {
			this.Get();
			this.gen.Emit(Op.CONST, 0); type = this.boolean; 
		} else this.SynErr(31);
		return type;
	}
	...
	ProcDecl() {
		let name_:string; let obj_:Obj; let adr:number; 
		this.Expect(9);
		name_ = this.Ident();
		obj_ = this.tab.NewObj(name_, this.proc, this.undef); obj_.adr =this.gen.pc;
	...
	VarDecl() {
		let name_:string;let type:number; 
		type = this.Type();
		name_ = this.Ident();
	...
	Stat() {
		let type:number;let name_:string;let obj_:Obj;
...
if (this.isLaTokenKindEqualTo(20)) {
				this.Get();
				this.gen.Emit(Op.JMP, 0); adr2 =this.gen.pc - 2;
		...
		case 21: {
			...
			if (type != this.boolean) this.SemErr("boolean type expected");
													 this.gen.Emit(Op.FJMP, 0); adr =this.gen.pc - 2; 
			this.Stat();
			this.gen.Emit(Op.JMP, loopstart);this.gen.Patch(adr,this.gen.pc); 
			break;
		}
		case 22: {
			this.Get();
